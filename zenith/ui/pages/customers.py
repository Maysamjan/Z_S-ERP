"""Customers page: list + create form, wired to PartyService."""
...
class CustomerDialog(QDialog):
    def __init__(self, ctx, parent=None):
        ...
        self.opening = self._money()
        self.limit = self._money()
    ...
    def _money(self):
        sb = QDoubleSpinBox(); sb.setMaximum(1_000_000_000); sb.setDecimals(2)
        return sb

    def _save(self):
        try:
            with session_scope(self.ctx.db) as session:
                PartyService(session).create_customer(
                    self.ctx.user, name=self.name.text(), phone=self.phone.text(),
                    address=self.address.text(),
                    opening_balance=Decimal(str(self.opening.value())),
